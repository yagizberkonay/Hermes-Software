...
const validate = (form, t) => {
  const errors = {};
  if (!form.name.trim()) errors.name = t.contact.errorRequired;
  if (!form.email.trim()) {
    errors.email = t.contact.errorRequired;
  } else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email)) {
    errors.email = t.contact.errorEmail;
  }
  if (!form.message.trim()) errors.message = t.contact.errorRequired;
  return errors;
};
...
export default function ContactModal({ open, onClose, estimate }) {
  ...
  const submit = async (e) => {
    e.preventDefault();
    if (sending || submittedRef.current) return;
    ...
    try {
      ...
      setStatus("success");
      track("contact_submit", { type: form.project_type || "unspecified" });
      ...
      {open && (
        ...
        >
          ...
          >
            ...
            {status === "success" ? (
              <div className="p-8 sm:p-12" role="status" aria-live="polite">
                <div className="border-[3px] border-[#111] bg-[#45B7D1] p-6">
                  <p className="font-display text-5xl leading-none mb-5">{t.contact.successLine1}</p>
                  <p className="font-display text-3xl">{t.contact.successLine2}</p>
                  <p className="font-semibold mt-6">{t.contact.emailDraft}</p>
                </div>
                <button
                  type="button"
                  onClick={onClose}
              ...
              <form onSubmit={submit} className="p-6 space-y-4" noValidate>
                ...
                {status === "error" && (
                  <p role="alert" className="font-mono-label text-[10px] font-bold text-[#FF5C5C] border-[3px] border-[#FF5C5C] px-3 py-2">
                    {t.contact.errorGeneric}
                  </p>
                )}
                {estimate && (
                  <p
                    className="font-mono-label text-[10px] font-bold bg-[#45B7D1] border-[3px] border-[#111] inline-block px-3 py-1.5"
                    data-testid="contact-estimate-badge"
                  >
                ...
                <div>
                  <label data-testid="contact-label-name" className="font-mono-label text-[10px] font-bold block mb-1.5" htmlFor="c-name">
                    {t.contact.name}
                  </label>
                  <input
                    id="c-name"
                ...
                <div>
                  <label data-testid="contact-label-email" className="font-mono-label text-[10px] font-bold block mb-1.5" htmlFor="c-email">
                    {t.contact.email}
                  </label>
                  <input
                    id="c-email"
                ...
                <div>
                  <label data-testid="contact-label-type" className="font-mono-label text-[10px] font-bold block mb-1.5" htmlFor="c-type">
                    {t.contact.type}
                  </label>
                ...
                <div>
                  <label data-testid="contact-label-message" className="font-mono-label text-[10px] font-bold block mb-1.5" htmlFor="c-msg">
                    {t.contact.message}
                  </label>
                  <textarea
                    id="c-msg"
                ...
                <div>
                  <label data-testid="contact-label-captcha" className="font-mono-label text-[10px] font-bold block mb-1.5" htmlFor="c-captcha">
